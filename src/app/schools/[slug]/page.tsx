...
import { MapPin, Star, IndianRupee, Phone, Mail, Calendar, Users,
  ...
  ChevronLeft, ChevronRight, X, Building2, GitCompare, Copy, Link2 as LinkIcon } from 'lucide-react';
...
export default function SchoolDetailPage() {
  ...
  const buildShareText = () => {
    if (!school) return '';
    const s = school;
    const url = typeof window !== 'undefined' ? window.location.href : `https://www.pickmyschool.in/schools/${s.id}`;
    const address = [s.address, s.city, s.state].filter(Boolean).join(', ');

    const academicFacilities = [
      s.hasLibrary && 'Library',
      s.hasComputerLab && 'Computer Lab',
      s.hasPhysicsLab && 'Physics Lab',
      s.hasChemistryLab && 'Chemistry Lab',
      s.hasBiologyLab && 'Biology Lab',
      s.hasMathsLab && 'Maths Lab',
      s.hasLanguageLab && 'Language Lab',
      s.hasRoboticsLab && 'Robotics Lab',
      s.hasStemLab && 'STEM Lab',
      s.hasAuditorium && 'Auditorium',
    ].filter(Boolean).join(', ');

    const sportsF = [
      s.hasPlayground && 'Playground',
      s.hasSwimmingPool && 'Swimming Pool',
      s.hasFitnessCentre && 'Fitness Centre',
      s.hasYoga && 'Yoga',
      s.hasMartialArts && 'Martial Arts',
      s.hasMusicDance && 'Music & Dance',
      s.hasHorseRiding && 'Horse Riding',
      s.sportsFacilities,
    ].filter(Boolean).join(', ');

    const techFacilities = [
      s.hasSmartBoard && 'Smart Boards',
      s.hasWifi && 'Wi-Fi Campus',
      s.hasCctv && 'CCTV Surveillance',
      s.hasElearning && 'E-Learning',
      s.hasAcClassrooms && 'AC Classrooms',
      s.hasAiTools && 'AI Tools',
    ].filter(Boolean).join(', ');
    ...
    const lines: string[] = [
      ...
      ...(s.schoolType ? [`🏛️ School Type: ${s.schoolType}`] : []),
      ...(s.k12Level ? [`📚 K-12 Level: ${s.k12Level}`] : []),
      ...(s.streamsAvailable ? [`🌟 Streams Available: ${s.streamsAvailable}`] : []),
      '',
      ...(academicFacilities ? [`✅ Academic Facilities: ${academicFacilities}`] : []),
      ...(sportsF ? [`⚽ Sports Facilities: ${sportsF}`] : []),
      ...(techFacilities ? [`💻 Technology: ${techFacilities}`] : []),
      '',
      ...(s.website ? [`🌐 Website: ${s.website}`] : []),
      ...((s.contactNumber || s.contactPhone) ? [`📞 Contact: ${s.contactNumber || s.contactPhone}`] : []),
      '',
      `🔗 View Full Profile:`,
      url,
      '',
      `Discover more top schools at www.pickmyschool.in`,
    ];
    return lines.join('\n');
  };
      ...
      <div className="pt-20">
        ...
        {showShareModal && school && (
          ...
          >
            ...
            >
              {/* Header */}
              <div className="bg-gradient-to-br from-cyan-500 to-teal-600 px-6 py-5 flex items-center gap-4">
                {school.logoUrl ? (
                  <img src={school.logoUrl} alt={school.name} className="w-14 h-14 rounded-2xl object-cover border-2 border-white/40 shadow-lg flex-shrink-0" />
                ) : (
                ...
                <div className="min-w-0">
                  <h3 className="text-white font-bold text-lg leading-tight truncate">{school.name}</h3>
                  {(school.city || school.state) && (
                    <p className="text-cyan-100 text-sm mt-0.5 truncate">{[school.city, school.state].filter(Boolean).join(', ')}</p>
                  )}
                </div>
                <button
                  onClick={() => setShowShareModal(false)}
                  className="ml-auto flex-shrink-0 w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="px-6 py-4 border-b border-gray-100">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Message Preview</p>
                <pre className="text-xs text-gray-600 bg-gray-50 rounded-xl p-3 max-h-48 overflow-y-auto whitespace-pre-wrap font-sans leading-relaxed border border-gray-100">
                  {buildShareText()}
                </pre>
              </div>
              ...
              <div className="px-6 py-5 grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-800 hover:bg-slate-900 text-white rounded-2xl font-semibold text-sm transition-all shadow-md hover:shadow-lg"
                >
                  <Copy size={16} />
                  Copy Details
                </button>
                <button
                  onClick={() => { navigator.clipboard.writeText(typeof window !== 'undefined' ? window.location.href : ''); toast.success('Link copied!'); }}
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-semibold text-sm transition-all"
                >
                  <LinkIcon size={16} />
                  Copy Link
                </button>
                <a
                  href={`mailto:?subject=${encodeURIComponent(`${school.name} — School Profile`)}&body=${encodeURIComponent(buildShareText())}`}
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl font-semibold text-sm transition-all shadow-md hover:shadow-lg"
                  onClick={() => setShowShareModal(false)}
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-semibold text-sm transition-all shadow-md hover:shadow-lg"
                  onClick={() => setShowShareModal(false)}
                >
                ...
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`${school.name} — Check out this school!\n${typeof window !== 'undefined' ? window.location.href : ''}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-black hover:bg-gray-900 text-white rounded-2xl font-semibold text-sm transition-all shadow-md hover:shadow-lg"
                  onClick={() => setShowShareModal(false)}
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-semibold text-sm transition-all shadow-md hover:shadow-lg"
                  onClick={() => setShowShareModal(false)}
                >
